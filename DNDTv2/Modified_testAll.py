...
from DNDTModel import DNDTClassifier
...
for d in datasets:
    ...
    if num_features > 8:
        ...
        selector = SelectKBest(score_func=mutual_info_classif, k=8)  # Select top 10 features
    ...
    train_indices, test_indices = train_test_split(np.arange(len(X)), test_size=0.33, random_state=42)

    X_train, X_test = X[train_index], X[test_index]
    y_train, y_test = y[train_index], y[test_index]

    # Initialize your model
    model = DNDTClassifier(n_features=num_features, n_classes=num_classes)
