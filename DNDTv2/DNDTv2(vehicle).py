...
data = arff.loadarff('../Datasets/vehicle0/vehicle0/vehicle0.data.arff')
    ...
    for var in range(nf):
        ...
        for i in range(train * int(tamano/X.shape[0])):
            ...
            if i % 200 == 0:
                ...
                if abs(aux - loss.detach().numpy()) < 10**(-7):
                    break
...
res = DNDTv2(x,y,2,2,10000, 4, x.shape[0])

with open('diccionario_entrenado(wis).pkl', 'wb') as f:
    pickle.dump(res, f)